...
elseif ($Method -eq "4") {
    ...
    # Fix search_path issue: Replace empty search_path with 'public'
    # pg_dump sets search_path to '' which breaks queries in connection poolers
    if ($backupContent -match "set_config\('search_path', ''") {
        Write-Host "  Fixing search_path (replacing empty with 'public')..." -ForegroundColor Yellow
        $backupContent = $backupContent -replace "SELECT pg_catalog\.set_config\('search_path', '', false\);", "SELECT pg_catalog.set_config('search_path', 'public', false);"
    }
...
"@
    ...
    Write-Host "  Wrapping in transaction for safe rollback on failure..." -ForegroundColor Yellow
    
    # Add search_path reset at the end to ensure clean connection state
    $resetSearchPathSQL = @"

-- Reset search_path to ensure connection pool gets clean connections
...
"@
    ...
    $restoreScript = $beginSQL + $truncateSQL + $backupContent + $commitSQL + $resetSearchPathSQL
}
